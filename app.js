...
(function () {
  ...
  // Starter question-type suggestions for the Review Log's autocomplete — grows
  // as the person types new ones in. "Mock" (full-sitting) rows aren't split
  // by section, so they draw from a merged list of all four.
  const REVIEW_TYPE_SEED = {
    QR: ["Ratios & Proportions", "Percentages", "Currency Conversion", "Data Interpretation (table)", "Data Interpretation (chart/graph)", "Distance/Speed/Time", "Averages (mean/median/mode)", "Menu/Price Problems", "Unit Conversion", "Best Value Comparison"],
    DM: ["Syllogism", "Venn Diagram", "Probabilistic Reasoning", "Interpreting Information", "Recognising Assumptions", "Evaluating Arguments", "Logical Puzzle", "Data Sufficiency", "Graph/Chart Interpretation", "Formal Logic (if/then)"],
    VR: ["True / False / Can't Tell", "Text Completion", "Reading Comprehension", "Skimming for Detail", "Author's Opinion or Tone", "Inference"],
    SJT: ["Appropriateness Judgement", "Importance Judgement", "Colleague Scenario", "Patient Scenario", "Senior/Supervisor Scenario", "Confidentiality Dilemma", "Prioritisation"],
  };
  REVIEW_TYPE_SEED.Mock = Array.from(new Set([].concat(REVIEW_TYPE_SEED.QR, REVIEW_TYPE_SEED.DM, REVIEW_TYPE_SEED.VR, REVIEW_TYPE_SEED.SJT)));
  ...
  /* ------------------------------ STORAGE ------------------------------ */
  function cloneReviewTypeSeed() {
    const out = {};
    Object.keys(REVIEW_TYPE_SEED).forEach((k) => { out[k] = REVIEW_TYPE_SEED[k].slice(); });
    return out;
  }
  ...
  function defaultData() {
    return {
      entries: [],
      reviewMocks: [], // {id, bucket, section, name, order, createdAt, collapsed}
      reviewLog: [], // question rows: {id, mockId, qNo, marks, questionType, whyWrong, order, createdAt}
      reviewTypeSuggestions: cloneReviewTypeSeed(),
  ...
  function normalizeDB(parsed) {
    ...
    // Migrate the old flat schema (rows carried bucket/section/mockName
    // directly, grouped only by matching text) into real mock entities the
    // first time this loads under the new structure.
    if (!Array.isArray(parsed.reviewMocks)) {
      const mockMap = {};
      const migrated = [];
      (parsed.reviewLog || []).forEach((r) => {
        if (r.mockId) { migrated.push(r); return; }
        const key = (r.bucket || "Mock") + "|" + (r.section || "") + "|" + (r.mockName || "Untitled");
        if (!mockMap[key]) {
          mockMap[key] = {
            id: uid(),
            bucket: r.bucket || "Mock",
            section: r.section || null,
            name: r.mockName || "Untitled",
            order: Object.keys(mockMap).length,
            createdAt: r.createdAt || Date.now(),
            collapsed: true,
          };
        }
        migrated.push({ id: r.id, mockId: mockMap[key].id, qNo: r.qNo, marks: r.marks, questionType: r.questionType, whyWrong: r.whyWrong, order: r.order || 0, createdAt: r.createdAt || Date.now() });
      });
      parsed.reviewMocks = Object.keys(mockMap).map((k) => mockMap[k]);
      parsed.reviewLog = migrated;
    }
    if (!Array.isArray(parsed.reviewLog)) parsed.reviewLog = [];
    parsed.reviewMocks.forEach((m, i) => {
      if (!m.id) m.id = uid();
      if (m.order === undefined || m.order === null) m.order = i;
      if (m.collapsed === undefined) m.collapsed = true;
    });
    if (!parsed.reviewTypeSuggestions) parsed.reviewTypeSuggestions = cloneReviewTypeSeed();
    Object.keys(REVIEW_TYPE_SEED).forEach((k) => {
      if (!Array.isArray(parsed.reviewTypeSuggestions[k])) parsed.reviewTypeSuggestions[k] = REVIEW_TYPE_SEED[k].slice();
    });
    parsed.reviewLog.forEach((r, i) => { if (r.order === undefined || r.order === null) r.order = i; });
  ...
  function dateAxisOptions() {
    return { grid: { display: false }, ticks: { maxTicksLimit: 10, callback: function (value) { return fmtDate(this.getLabelForValue(value)); } } };
  }
  ...
  if (!CHART_LIB_MISSING) {
    ...
    // Room around the plotted area so a point/line sitting exactly at an
    // axis min or max (very common here — scores pinned to 300–900, %
    // pinned to 0–100, etc.) doesn't get its marker or stroke sliced off
    // by the canvas edge. Without this, "perfect score" points were
    // literally invisible — half-drawn off the top of the canvas.
    Chart.defaults.layout.padding = { top: 14, right: 14, bottom: 4, left: 4 };
    ...
    // Flatter, quieter grid: horizontal reference lines only, no axis
    // border, so multi-line charts read as "data on a soft backdrop"
    // rather than fighting a boxed-in grid for attention.
    Chart.defaults.scale.grid.color = "rgba(124,106,111,0.10)";
  ...
  // Per-section line styling for multi-line charts (QR/DM/VR/SJT often
  // share the same chart). Color alone isn't always enough to tell lines
  // apart at a glance — QR and SJT in particular sit close on the
  // palette — so each section also gets its own point shape and a subtle
  // dash pattern on top of its color, the way a good legend would.
  const SECTION_POINT_STYLE = { QR: "circle", DM: "rectRot", VR: "triangle", SJT: "rect" };
  const SECTION_DASH = { QR: [], DM: [], VR: [], SJT: [] };

  // Builds a consistently-styled line dataset for one section, so every
  // multi-line chart in the app looks and behaves the same way.
  function sectionLineDataset(sec, data, extra) {
    return Object.assign(
      {
        ...
        pointBackgroundColor: SECTION_COLORS[sec],
        pointBorderColor: "#FFFFFF",
        pointStyle: SECTION_POINT_STYLE[sec] || "circle",
        borderDash: SECTION_DASH[sec] || [],
        tension: 0,
        spanGaps: true,
      },
      extra || {}
  ...
  let practiceFilter = "overall"; // "timed" | "untimed" | "overall"
  ...
  function renderPage(key) {
    ...
    else if (key === "review") renderReviewPage();
  ...
  function renderOverallTrendChart() {
    ...
    const datasets = COGNITIVE_SECTIONS.map((sec) =>
      sectionLineDataset(sec, seriesAlignedToTimeline(bySection(sec), timeline, (e) => e.scaled))
    );
    makeChart(canvas, {
      type: "line",
  ...
  function renderSectionPage(sec) {
    ...
    // Trend chart — Timed, Untimed, Overall; Overall drawn last so it sits on top
    const allTimeline = buildAttemptTimeline(entries);
    const allLabels = timelineLabels(allTimeline);
    const TIMED_CATS = ["Full Mock", "Medify Mock", "UCAT Official Mock", "Mini-Mock", "Timed Practice", "UCAT Official Practice"];
    const subGroups = [
      { label: "Timed",   cats: TIMED_CATS,            color: "#5B9BD5" },
      { label: "Untimed", cats: ["Untimed Practice"],  color: "#5FA876" },
    ];
    const subDatasets = subGroups.map((g) => {
      const sub = entries.filter((e) => g.cats.includes(e.category));
      return {
        label: g.label,
        data: seriesAlignedToTimeline(sub, allTimeline, (e) => e.scaled),
        borderColor: g.color, backgroundColor: g.color,
        showLine: true, spanGaps: true, borderWidth: 1.5, tension: 0,
        pointRadius: 6, pointHoverRadius: 8,
        pointStyle: g.label === "Timed" ? "circle" : "rectRot",
        order: 1,
      };
    });
    makeChart(page.querySelector(".chart-trend"), {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: "Overall",
            data: entries.map((e) => e.scaled),
            borderColor: SECTION_COLORS[sec], backgroundColor: SECTION_COLORS[sec],
            borderWidth: 2.5, pointRadius: 0, spanGaps: true, order: 2,
          },
        ],
      },
    ...
    makeChart(page.querySelector(".chart-dist"), {
      ...
      data: {
        ...
        datasets: [{ label: "Attempts", data: Object.values(buckets), backgroundColor: SECTION_SOFT[sec], borderColor: SECTION_COLORS[sec], borderWidth: 2 }],
      },
  ...
  function renderSjtSectionPage() {
    ...
    if (withRaw.length) {
      const TIMED_CATS = ["Full Mock", "Medify Mock", "UCAT Official Mock", "Mini-Mock", "Timed Practice", "UCAT Official Practice"];
      const sjtTimeline = buildAttemptTimeline(withRaw);
      const sjtLabels = timelineLabels(sjtTimeline);
      const sjtSubGroups = [
        { label: "Timed",   cats: TIMED_CATS,           color: "#5B9BD5" },
        { label: "Untimed", cats: ["Untimed Practice"], color: "#5FA876" },
      ];
      const sjtSubDatasets = sjtSubGroups.map((g) => {
        const sub = withRaw.filter((e) => g.cats.includes(e.category));
        return {
          label: g.label,
          data: seriesAlignedToTimeline(sub, sjtTimeline, (e) => e.raw),
          borderColor: g.color, backgroundColor: g.color,
          showLine: true, spanGaps: true, borderWidth: 1.5, tension: 0,
          pointRadius: 6, pointHoverRadius: 8,
          pointStyle: g.label === "Timed" ? "circle" : "rectRot",
      ...
      makeChart(page.querySelector(".chart-trend"), {
        ...
        data: {
          ...
          datasets: [
            ...
            {
              label: "Overall",
              data: withRaw.map((e) => e.raw),
              borderColor: SECTION_COLORS.SJT, backgroundColor: SECTION_COLORS.SJT,
              borderWidth: 2.5, pointRadius: 0, spanGaps: true, order: 2,
            },
          ],
        },
        options: { scales: { x: dateAxisOptions(), y: { min: 0, max: 69, title: { display: true, text: "Raw mark (/69)" } } }, plugins: { legend: { position: "bottom" } } },
      });
    } else {
    ...
    makeChart(page.querySelector(".chart-dist"), {
      ...
      data: {
        ...
        datasets: [{ label: "Attempts", data: bandCounts, backgroundColor: ["#E1ECE2", "#F3EAD2", "#F6E2D7", "#F6E0DE"], borderColor: SECTION_COLORS.SJT, borderWidth: 2 }],
      },
      options: { plugins: { legend: { display: false } }, scales: { y: { ticks: { stepSize: 1 } } } },
    });
  ...
  function renderFullMocksPage() {
    ...
    makeChart(document.getElementById("chartFmOverall"), {
      ...
      data: {
        ...
        datasets: [{ label: "Cognitive total (/2700)", data: sittings.map((s) => cognitiveTotal(s.sections)), borderColor: "#3F3136", backgroundColor: "#3F3136", tension: 0, pointRadius: 5 }],
      },
    ...
    if (sjtSittings.length) {
      makeChart(document.getElementById("chartFmSjtBand"), {
        ...
        data: {
          labels: sittings.map((s) => fmtDate(s.date) + " · " + s.testName),
          datasets: [{ label: "SJT band", data: sittings.map((s) => s.sjtBand || null), borderColor: SECTION_COLORS.SJT, backgroundColor: SECTION_COLORS.SJT, stepped: true, pointRadius: 6, spanGaps: true }],
        },
    ...
    const overall = filterEntries({ categories: ["Timed Practice", "Untimed Practice"] });
    const timedOrUntimed = timed.length || untimed.length;
    if (!timedOrUntimed) {
      emptyState(document.getElementById("chartTimedUntimed"));
    } else {
      makeChart(document.getElementById("chartTimedUntimed"), {
        ...
        data: {
          ...
          datasets: [
            ...
            { label: "Overall avg", data: COGNITIVE_SECTIONS.map((s) => fmt0(mean(overall.filter((e) => e.section === s).map((e) => e.scaled))) || 0), backgroundColor: "#604D53" },
          ],
        },
        options: { scales: { y: { min: 300, max: 900 } }, plugins: { legend: { position: "bottom" } } },
      });
      ...
      makeChart(trendCanvas, {
        ...
        data: {
          ...
          datasets: COGNITIVE_SECTIONS.map((sec) =>
            sectionLineDataset(sec, seriesAlignedToTimeline(entries.filter((e) => e.section === sec), timeline, (e) => e.scaled))
          ),
        },
        options: {
          interaction: { mode: "point", intersect: true },
          scales: { x: dateAxisOptions(), y: { min: 300, max: 900 } },
          plugins: { legend: { position: "bottom" } },
        },
      });
      makeChart(avgCanvas, {
  ...
  function renderPracticePage() {
    const TIMED_CATS = ["Timed Practice", "Mini-Mock"];
    const UNTIMED_CATS = ["Untimed Practice"];
    const ALL_CATS = ["Timed Practice", "Untimed Practice", "Mini-Mock"];
    const cats = practiceFilter === "timed" ? TIMED_CATS : practiceFilter === "untimed" ? UNTIMED_CATS : ALL_CATS;
    const practiceEntries = filterEntries({ categories: cats });
    ...
    makeChart(accCanvas, {
      ...
      data: {
        ...
        datasets: SECTIONS.map((sec) =>
          sectionLineDataset(sec, seriesAlignedToTimeline(practiceEntries.filter((e) => e.section === sec), timeline, (e) => e.accuracy))
        ),
      },
      options: {
        interaction: { mode: "point", intersect: true },
        scales: { x: dateAxisOptions(), y: { min: 0, max: 100, title: { display: true, text: "Raw mark %" } } },
        plugins: { legend: { position: "bottom" } },
      },
    });
    ...
    makeChart(compCanvas, {
      ...
      data: {
        labels,
        datasets: SECTIONS.map((sec) =>
          sectionLineDataset(sec, seriesAlignedToTimeline(practiceEntries.filter((e) => e.section === sec && e.completion !== null), timeline, (e) => e.completion))
        ),
      },
      options: {
        interaction: { mode: "point", intersect: true },
        scales: { x: dateAxisOptions(), y: { min: 0, max: 100, title: { display: true, text: "Completion %" } } },
        plugins: { legend: { position: "bottom" } },
      },
    });
    ...
    makeChart(timeCanvas, {
      ...
      data: {
        labels,
        datasets: SECTIONS.map((sec) =>
          sectionLineDataset(sec, seriesAlignedToTimeline(practiceEntries.filter((e) => e.section === sec && e.time && e.qCount), timeline, (e) => Math.round((e.time * 60) / e.qCount)))
        ),
      },
      options: {
        interaction: { mode: "point", intersect: true },
        scales: { x: dateAxisOptions(), y: { title: { display: true, text: "Seconds / question" } } },
        plugins: { legend: { position: "bottom" } },
      },
    });
  ...
  /* =========================================================================
     REVIEW LOG (question-by-question breakdown — separate from score entries)
     Structure: DB.reviewMocks holds each mock/mini-mock as a real entity
     (id, bucket, section, name, collapsed). DB.reviewLog holds question rows,
     each pointing at its parent via mockId. Grouping is structural (by id),
     never by matching name text, so renaming a mock can't split or merge
     groups by accident.
     ========================================================================= */
  let reviewBucket = "Mock";
  let reviewSection = "QR";

  function reviewVisibleMocks() {
    const mocks = DB.reviewMocks.filter((m) => m.bucket === reviewBucket && (reviewBucket !== "Mini-Mock" || m.section === reviewSection));
    mocks.sort((a, b) => mockLastActivity(b) - mockLastActivity(a));
    return mocks;
  }

  function mockQuestions(mockId) {
    return DB.reviewLog.filter((r) => r.mockId === mockId).sort((a, b) => (a.order || 0) - (b.order || 0));
  }

  function mockLastActivity(m) {
    const qs = mockQuestions(m.id);
    if (!qs.length) return m.createdAt || 0;
    return Math.max.apply(null, qs.map((q) => q.createdAt || 0));
  }

  function reviewSuggestionsFor(field) {
    if (field === "questionType") {
      return reviewBucket === "Mini-Mock" ? DB.reviewTypeSuggestions[reviewSection] || [] : DB.reviewTypeSuggestions.Mock || [];
    }
    return [];
  }

  function addReviewTypeSuggestion(value) {
    if (!value) return;
    const key = reviewBucket === "Mini-Mock" ? reviewSection : "Mock";
    const list = DB.reviewTypeSuggestions[key];
  ...
  function plainCell(field, id, value) {
    return "<textarea class='cell-textarea' rows='1' data-field='" + field + "' data-row='" + id + "'>" + esc(value || "") + "</textarea>";
  }
  function acCell(field, id, value) {
    return "<div class='ac-wrap'><textarea class='cell-textarea ac-input' rows='1' data-field='" + field + "' data-row='" + id + "'>" + esc(value || "") + "</textarea><div class='ac-dropdown hidden'></div></div>";
  }

  function reviewInputRowHTML(mockId) {
    return "<tr class='review-row review-input-row' data-id='new' data-mock-id='" + mockId + "'>" +
      "<td class='rv-handle rv-handle-add'>+</td>" +
      "<td>" + plainCell("qNo", "new", "") + "</td>" +
      "<td>" + plainCell("marks", "new", "") + "</td>" +
      "<td>" + acCell("questionType", "new", "") + "</td>" +
      "<td>" + plainCell("whyWrong", "new", "") + "</td>" +
      "<td><button class='btn-ghost' data-add-question='" + mockId + "' type='button'>Add</button></td>" +
      "</tr>";
  }

  function reviewRowHTML(r) {
    return "<tr class='review-row' data-id='" + r.id + "' data-mock-id='" + r.mockId + "'>" +
      "<td class='rv-handle' draggable='true' title='Drag to reorder'>⠿</td>" +
      "<td>" + plainCell("qNo", r.id, r.qNo) + "</td>" +
      "<td>" + plainCell("marks", r.id, r.marks) + "</td>" +
      "<td>" + acCell("questionType", r.id, r.questionType) + "</td>" +
      "<td>" + plainCell("whyWrong", r.id, r.whyWrong) + "</td>" +
      "<td><button class='del-btn' data-rv-del='" + r.id + "'>Delete</button></td>" +
      "</tr>";
  }

  function reviewMockCardHTML(m) {
    const qs = mockQuestions(m.id);
    const expanded = !m.collapsed;
    let html = "<div class='review-mock" + (expanded ? " expanded" : "") + "' data-mock-id='" + m.id + "'>";
    html += "<div class='review-mock-header' data-mock-toggle='" + m.id + "'>";
    html += "<span class='review-mock-chevron'>" + (expanded ? "▾" : "▸") + "</span>";
    html += "<input class='review-mock-name' data-mock-name='" + m.id + "' value=\"" + esc(m.name) + "\" />";
    html += "<span class='review-mock-count'>" + qs.length + " question" + (qs.length === 1 ? "" : "s") + "</span>";
    html += "<button class='del-btn review-mock-del' data-mock-del='" + m.id + "'>Delete mock</button>";
    html += "</div>";
    if (expanded) {
      html += "<div class='review-mock-body'><div class='table-wrap'><table class='data-table review-table'><thead><tr>" +
        "<th class='rv-col-handle'></th><th>Q No.</th><th>Marks</th><th>Question type</th><th>Why wrong</th><th></th>" +
        "</tr></thead><tbody data-mock-body='" + m.id + "'>";
      html += reviewInputRowHTML(m.id);
      qs.forEach((q) => { html += reviewRowHTML(q); });
  ...
  function renderReviewPage() {
    document.querySelectorAll(".review-bucket-btn").forEach((b) => b.classList.toggle("active", b.dataset.bucket === reviewBucket));
    document.getElementById("reviewSectionBar").classList.toggle("hidden", reviewBucket !== "Mini-Mock");
    document.querySelectorAll(".review-section-btn").forEach((b) => b.classList.toggle("active", b.dataset.section === reviewSection));

    const mocks = reviewVisibleMocks();
    let html = "<div class='review-add-mock-bar'>" +
      "<textarea class='cell-textarea' id='newMockNameInput' rows='1' placeholder='New mock name…'></textarea>" +
      "<button class='btn-primary' id='addMockBtn' type='button'>+ Add mock</button>" +
      "</div>";
    if (!mocks.length) {
      html += "<p class='review-empty'>No mocks logged yet for this view — add one above to start logging questions.</p>";
    } else {
      mocks.forEach((m) => { html += reviewMockCardHTML(m); });
    }
    const container = document.getElementById("reviewMocksContainer");
    container.innerHTML = html;
    container.querySelectorAll("textarea.cell-textarea").forEach(autoGrow);
    bindReviewEvents(container);
  }

  function showAcDropdown(textareaEl) {
    const wrap = textareaEl.closest(".ac-wrap");
    const dd = wrap.querySelector(".ac-dropdown");
    const value = textareaEl.value;
    const lastNL = value.lastIndexOf("\n");
    const fragment = value.slice(lastNL + 1).trim().toLowerCase();
    const all = reviewSuggestionsFor(textareaEl.dataset.field);
    const matches = fragment ? all.filter((s) => s.toLowerCase().indexOf(fragment) !== -1) : all;
    if (!matches.length) {
      dd.classList.add("hidden");
      dd.innerHTML = "";
      return;
    }
    dd.innerHTML = matches.slice(0, 8).map((s) => "<div class='ac-item' data-value=\"" + esc(s) + "\">" + esc(s) + "</div>").join("");
    dd.classList.remove("hidden");
    dd.querySelectorAll(".ac-item").forEach((item) => {
      item.addEventListener("mousedown", (e) => {
        e.preventDefault();
        const val = textareaEl.value;
        const lastNL2 = val.lastIndexOf("\n");
        const prefix = lastNL2 >= 0 ? val.slice(0, lastNL2 + 1) : "";
        textareaEl.value = prefix + item.dataset.value;
        dd.classList.add("hidden");
        dd.innerHTML = "";
        autoGrow(textareaEl);
        if (textareaEl.dataset.row !== "new") updateReviewField(textareaEl.dataset.row, textareaEl.dataset.field, textareaEl.value);
        if (textareaEl.dataset.field === "questionType") { addReviewTypeSuggestion(item.dataset.value); save(); }
      });
    });
  }

  function hideAcDropdown(textareaEl) {
    const wrap = textareaEl.closest(".ac-wrap");
    wrap.querySelector(".ac-dropdown").classList.add("hidden");
  }

  function updateReviewField(id, field, value) {
    const row = DB.reviewLog.find((r) => r.id === id);
    if (!row) return;
    row[field] = value;
    save();
  }

  function persistMockOrder(mockId) {
    const body = document.querySelector('tbody[data-mock-body="' + mockId + '"]');
    if (!body) return;
    const rowEls = Array.from(body.querySelectorAll(".review-row:not(.review-input-row)"));
    rowEls.forEach((rowEl, i) => {
      const row = DB.reviewLog.find((r) => r.id === rowEl.dataset.id);
      if (row) row.order = i;
    });
    save();
  }

  function handleReviewAddQuestion(mockId, body) {
    const scope = body.querySelector('tbody[data-mock-body="' + mockId + '"]');
    const getVal = (field) => scope.querySelector("textarea[data-row='new'][data-field='" + field + "']").value.trim();
    const qNo = getVal("qNo");
    const marks = getVal("marks");
    const questionType = getVal("questionType");
    const whyWrong = getVal("whyWrong");
    if (!qNo) {
      alert("Q No. is required.");
      return;
    }
    const existing = mockQuestions(mockId);
    const nextOrder = existing.length ? Math.max.apply(null, existing.map((r) => r.order || 0)) + 1 : 0;
    DB.reviewLog.push({
      id: uid(),
      mockId,
      qNo, marks, questionType, whyWrong,
      order: nextOrder,
      createdAt: Date.now(),
    });
    addReviewTypeSuggestion(questionType);
    save();
    renderReviewPage();
  }

  function handleAddMock() {
    const input = document.getElementById("newMockNameInput");
    const name = input.value.trim();
    if (!name) {
      alert("Enter a mock name first.");
      return;
    }
    const mock = {
      id: uid(),
      bucket: reviewBucket,
      section: reviewBucket === "Mini-Mock" ? reviewSection : null,
      name,
      order: DB.reviewMocks.length,
      createdAt: Date.now(),
      collapsed: false, // auto-expand so the first question can be added right away
    };
    DB.reviewMocks.push(mock);
    save();
    renderReviewPage();
    const firstField = document.querySelector('tbody[data-mock-body="' + mock.id + '"] textarea[data-row="new"][data-field="qNo"]');
    if (firstField) firstField.focus();
  }

  function bindReviewEvents(container) {
    const addMockBtn = document.getElementById("addMockBtn");
    if (addMockBtn) addMockBtn.addEventListener("click", handleAddMock);
    const newMockInput = document.getElementById("newMockNameInput");
    if (newMockInput) {
      autoGrow(newMockInput);
      newMockInput.addEventListener("input", () => autoGrow(newMockInput));
      newMockInput.addEventListener("keydown", (e) => {
        if (e.key === "Enter") { e.preventDefault(); handleAddMock(); }
      });
    }

    container.querySelectorAll("[data-mock-toggle]").forEach((header) => {
      header.addEventListener("click", (e) => {
        if (e.target.closest(".review-mock-name") || e.target.closest(".review-mock-del")) return;
        const m = DB.reviewMocks.find((mm) => mm.id === header.dataset.mockToggle);
        if (!m) return;
    ...
    container.querySelectorAll("[data-mock-name]").forEach((input) => {
      input.addEventListener("click", (e) => e.stopPropagation());
      input.addEventListener("blur", () => {
        const m = DB.reviewMocks.find((mm) => mm.id === input.dataset.mockName);
        if (!m) return;
        const val = input.value.trim();
        if (val) { m.name = val; save(); }
        else input.value = m.name;
      });
    ...
    container.querySelectorAll("[data-mock-del]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        if (!confirm("Delete this mock and all its logged questions? This can't be undone.")) return;
        const id = btn.dataset.mockDel;
        DB.reviewMocks = DB.reviewMocks.filter((m) => m.id !== id);
        DB.reviewLog = DB.reviewLog.filter((r) => r.mockId !== id);
        save();
        renderReviewPage();
      });
    });
    ...
    container.querySelectorAll("[data-add-question]").forEach((btn) => {
      btn.addEventListener("click", () => handleReviewAddQuestion(btn.dataset.addQuestion, container));
    });

    container.querySelectorAll("textarea.cell-textarea").forEach((el) => {
      el.addEventListener("input", () => {
        autoGrow(el);
        if (el.dataset.row && el.dataset.row !== "new") updateReviewField(el.dataset.row, el.dataset.field, el.value);
        if (el.classList.contains("ac-input")) showAcDropdown(el);
      });
      if (el.classList.contains("ac-input")) {
        el.addEventListener("focus", () => showAcDropdown(el));
        el.addEventListener("blur", () => {
          setTimeout(() => hideAcDropdown(el), 150);
          if (el.dataset.field === "questionType" && el.dataset.row !== "new") {
            addReviewTypeSuggestion(el.value.trim());
            save();
          }
        });
      }
    ...
    container.querySelectorAll("[data-rv-del]").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (!confirm("Delete this review entry?")) return;
        DB.reviewLog = DB.reviewLog.filter((r) => r.id !== btn.dataset.rvDel);
        save();
        renderReviewPage();
      });
    });
    ...
    let draggedTr = null;
    container.querySelectorAll(".review-row:not(.review-input-row)").forEach((tr) => {
      const handle = tr.querySelector(".rv-handle");
      ...
      handle.addEventListener("dragend", () => {
        if (!draggedTr) return;
        draggedTr.classList.remove("dragging");
        persistMockOrder(draggedTr.dataset.mockId);
        draggedTr = null;
      });
      tr.addEventListener("dragover", (e) => {
        if (!draggedTr || draggedTr === tr || tr.dataset.mockId !== draggedTr.dataset.mockId) return;
        e.preventDefault();
        const rect = tr.getBoundingClientRect();
        const before = (e.clientY - rect.top) < rect.height / 2;
        tr.parentNode.insertBefore(draggedTr, before ? tr : tr.nextSibling);
      });
    });
  }
  ...
  function init() {
    ...
    // practice lab filter (Timed / Untimed / Overall)
    document.querySelectorAll(".practice-filter-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        practiceFilter = btn.dataset.filter;
        document.querySelectorAll(".practice-filter-btn").forEach((b) => b.classList.toggle("active", b === btn));
        renderPracticePage();
      });
    });

    // review log bucket / section toggle
    document.querySelectorAll(".review-bucket-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        reviewBucket = btn.dataset.bucket;
        renderReviewPage();
      });
    });
    document.querySelectorAll(".review-section-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        reviewSection = btn.dataset.section;
        renderReviewPage();
      });
    });
